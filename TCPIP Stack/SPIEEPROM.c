...
static unsigned char EEPROMBuffer[EEPROM_BUFFER_SIZE];
static unsigned char *EEPROMBufferPtr;
...
void XEEInit(void)
{
	...
	EEPROM_SPIBRG = (PERIPHERAL_FREQ - 1ul) / 2ul / EEPROM_MAX_SPI_FREQ;
...
/*********************************************************************
 * Function:        unsigned char XEERead(void)
 *
 * PreCondition:    XEEInit() && XEEBeginRead() are already called.
 *
 * Input:           None
 *
 * Output:          unsigned char that was read
 *
 * Side Effects:    None
 *
...
static void DoWrite(void)
{
	...
	BytesToWrite = (unsigned char) (EEPROMBufferPtr - EEPROMBuffer);
