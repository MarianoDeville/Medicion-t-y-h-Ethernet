...
void SNTPClient(void)
{
	...
	case SM_HOME:
		...
		DNSResolveROM((const unsigned char *) NTP_SERVER, DNS_TYPE_A);
		dwTimer = TickGet();
	...
	case SM_UDP_SEND:
		...
		UDPPutArray((unsigned char *) &pkt, sizeof(pkt));
		UDPFlush();
	...
	case SM_UDP_RECV:
		...
		w = UDPGetArray((unsigned char *) &pkt, sizeof(pkt));
		UDPClose(MySocket);
		dwTimer = TickGetDiv64K();
		SNTPState = SM_WAIT;
		...
		if (((unsigned char *) &pkt.tx_ts_fraq)[0] & 0x80)
			dwSNTPSeconds++;
	...
	case SM_SHORT_WAIT:
		...
		if (TickGetDiv64K() - dwTimer >
			(NTP_FAST_QUERY_INTERVAL / 65536ull))
			SNTPState = SM_HOME;
		break;
