...
static unsigned char *DNSHostName;
static const unsigned char *DNSHostNameROM;
static unsigned char RecordType;
...
static void DNSPutString(unsigned char *String);
static void DNSGetString(unsigned char *String);

// const function variants for PIC18
#if defined(__18CXX)
static void DNSPutROMString(const unsigned char *String);
#else
#define DNSPutROMString(a)	DNSPutString((unsigned char*)a)
#endif
...
/*********************************************************************
 * Function:        void DNSResolve(unsigned char *Hostname, unsigned char RecordType)
 *
 * PreCondition:    Stack is initialized()
 *
...
BOOL DNSIsResolved(IP_ADDR * HostIP)
{
	...
	case DNS_ARP_RESOLVE3:
		if (!ARPIsResolved
			(&AppConfig.PrimaryDNSServer, &ResolvedInfo.MACAddr)) {
	...
	case DNS_GET_RESULT3:
		...
		while (DNSHeader.Answers.Val--) {
			...
			if (DNSAnswerHeader.ResponseType.Val == 0x0001u && DNSAnswerHeader.ResponseClass.Val == 0x0001u &&	// Internet class
...
static void DNSPutString(unsigned char *String)
{
	...
	while (1) {
		...
		Len = (unsigned char) (RightPtr - String - 1);
		UDPPut(Len);
