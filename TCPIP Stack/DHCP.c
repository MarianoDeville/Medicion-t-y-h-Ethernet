...
#endif
//static unsigned char tempHostName[16];
static union {
	struct {
		char IPAddress:1;
		char Gateway:1;
...
static unsigned char _DHCPReceive(void);
static void _DHCPSend(unsigned char messageType, BOOL bRenewing);
/*********************************************************************
 ...
 *
 * Note:            If the board was configured by DHCP before 
 *					DHCPDisable() was called, then the configuration 
 *					will continue to be used, but the DHCP module 
 *					will no longer perform any lease renewals or 
 *					otherwise.  In other words, if the lease expires 
 *					the board will still be using the old IP address 
 *					and the DHCP server may decide to give the IP 
 *					address to someone else.  The application should 
 *					change the current IP address and other 
 *					configuration with static information provided by 
 *					the user following a call to DHCPDisable().
 ********************************************************************/
...
void DHCPTask(void)
{
	...
	case SM_DHCP_SEND_DISCOVERY:
		if (UDPIsPutReady(DHCPSocket) < 300)
			break;
...
static unsigned char _DHCPReceive(void)
{
	...
	if (v == BOOT_REPLY) {
		...
		if (DHCPFlags.bits.bOfferReceived) {
			...
			UDPGetArray((unsigned char *) &tempIPAddress,
		...
		do {
			...
			case DHCP_SUBNET_MASK:
				...
				if (v == 4u) {
					...
					if (DHCPFlags.bits.bOfferReceived) {
						...
						UDPGetArray((unsigned char *) &tempMask,sizeof(tempMask));
			...
			case DHCP_ROUTER:
				...
				if (j >= 4u) {
					...
					if (DHCPFlags.bits.bOfferReceived) {
						...
						UDPGetArray((unsigned char *) &tempGateway,sizeof(tempGateway));
			...
			case DHCP_DNS:
				...
				// Check to see if this is the first offer
				if (!DHCPFlags.bits.bOfferReceived) {
					UDPGetArray((unsigned char *) &tempDNS,sizeof(tempDNS));
				...
				if (j >= 4u) {
					// Check to see if this is the first offer
					if (!DHCPFlags.bits.bOfferReceived) {
						UDPGetArray((unsigned char *) &tempDNS2,sizeof(tempDNS2));
...
static void _DHCPSend(unsigned char messageType, BOOL bRenewing)
{
	...
	if ((messageType == DHCP_REQUEST_MESSAGE) && bRenewing) {
		UDPPutArray((unsigned char *) &tempIPAddress,sizeof(tempIPAddress));
	} else {
		UDPPut(0x00);
		UDPPut(0x00);
		UDPPut(0x00);
	...
	UDPPutArray((unsigned char *) &AppConfig.MyMACAddr,sizeof(AppConfig.MyMACAddr));
	...
	if (((messageType == DHCP_REQUEST_MESSAGE) && !bRenewing) ||
		...
		UDPPutArray((unsigned char *) &tempIPAddress,DHCP_PARAM_REQUEST_IP_ADDRESS_LEN);
	}
