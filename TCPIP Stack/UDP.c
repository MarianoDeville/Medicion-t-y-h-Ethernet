...
UDP_SOCKET UDPOpen(UDP_PORT localPort,
				   NODE_INFO * remoteNode, UDP_PORT remotePort)
{
	volatile UDP_SOCKET s;
	UDP_SOCKET_INFO *p;
	...
	for (s = 0; s < MAX_UDP_SOCKETS; s++) {
		if (p->localPort == INVALID_UDP_PORT) {
			...
			if (localPort == 0x0000u) {
				if (NextPort > LOCAL_UDP_PORT_END_NUMBER
...
WORD UDPIsPutReady(UDP_SOCKET s)
{
	...
	return MAC_TX_BUFFER_SIZE - sizeof(IP_HEADER) - sizeof(UDP_HEADER) -
		UDPTxCount;
}

/*********************************************************************
 * Function:        BOOL UDPPut(unsigned char v)
 *
...
/*********************************************************************
 * Function:        WORD UDPPutArray(unsigned char *cData, WORD wDataLen)
 *
 * PreCondition:    UDPIsPutReady() > 0 with desired UDP socket
 *                  that is to be loaded.
 *
...
{
	return strData + UDPPutROMArray(strData,
									strlenpgm((const char *) strData));
}
#endif
...
void UDPFlush(void)
{
	...
	MACPutArray((unsigned char *) &h, sizeof(h));

	// Calculate the final UDP checksum and write it in, if enabled
#if defined(UDP_USE_TX_CHECKSUM)
	{
		wReadPtrSave =
			MACSetReadPtr(BASE_TX_ADDR + sizeof(ETHER_HEADER) +
						  sizeof(IP_HEADER));
		...
		MACPutArray((unsigned char *) &wChecksum, sizeof(wChecksum));
	}
#endif
...
/*********************************************************************
 * Function:        BOOL UDPGet(unsigned char *v)
 *
...
/*********************************************************************
 * Function:        WORD UDPGetArray(unsigned char *cData, WORD wDataLen)
 *
 * PreCondition:    UDPInit() is already called     AND
 *                  UDPIsGetReady(s) > 0
 *
...
BOOL UDPProcess(NODE_INFO * remoteNode, IP_ADDR * localIP, WORD len)
{
	...
	// Retrieve UDP header.
	MACGetArray((unsigned char *) &h, sizeof(h));
	...
	if (h.Checksum) {
		...
		checksums.w[0] = ~CalcIPChecksum((unsigned char *) &pseudoHeader,
										 sizeof(pseudoHeader));
