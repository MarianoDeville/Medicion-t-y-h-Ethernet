...
static unsigned char _tftpRetries;
...
#if defined(__18CXX)
static void _TFTPSendROMFileName(TFTP_OPCODE opcode, const char *fileName);
#endif
 ...
 ********************************************************************/
void TFTPOpenFile(unsigned char *fileName, TFTP_FILE_MODE mode)
{
...
#if defined(__18CXX)
void TFTPOpenROMFile(const unsigned char *fileName, TFTP_FILE_MODE mode)
{
	DEBUG(printf("Opening file...\n"));
...
TFTP_RESULT TFTPIsGetReady(void)
{
	...
	case SM_TFTP_WAIT_FOR_DATA:
		...
		if (opCode.Val == TFTP_OPCODE_DATA) {
			...
			else if (MutExVar.group2._tftpBlockNumber.Val >
					 blockNumber.Val) {
				DEBUG(printf
					  ("TFTPIsGetReady(): "
	...
	case SM_TFTP_READY:
		...
		// determine if block is less than 512 bytes long or not.
				 ...
				 MutExVar.group2._tftpBlockLength.v[1] <
				 TFTP_BLOCK_SIZE_MSB)
...
/*********************************************************************
 * Function:        unsigned char TFTPGet(void)
 *
 * PreCondition:    TFTPOpenFile() is called with TFTP_FILE_MODE_READ
...
/*********************************************************************
 * Function:        void TFTPPut(unsigned char c)
 *
 * PreCondition:    TFTPOpenFile() is called with TFTP_FILE_MODE_WRITE
