...
static const unsigned char strTitle[] = "\x1b[2J\x1b[31m\x1b[1m"	// 2J is clear screen, 31m is red, 1m is bold
	...
	"Login: ";
static const unsigned char strPassword[] = "Password: \xff\xfd\x2d";	// DO Suppress Local Echo (stop telnet client from printing typed characters)
static const unsigned char strAccessDenied[] = "\r\nAccess denied\r\n\r\n";
static const unsigned char strAuthenticated[] =
	"\r\nLogged in successfully\r\n\r\n" "\r\nPress 'q' to quit\r\n";
static const unsigned char strDisplay[] =
	"\r\nAnalog:             1023" "\r\nButtons:         3 2 1 0"
	"\r\nLEDs:    7 6 5 4 3 2 1 0";
static const unsigned char strGoodBye[] = "\r\n\r\nGoodbye!\r\n";
...
void TelnetTask(void)
{
	...
	case SM_HOME:
		...
		MySocket =
			TCPOpen(0, TCP_OPEN_SERVER, TELNET_PORT, TCP_PURPOSE_TELNET);
	...
	case SM_PRINT_LOGIN:
		...
		if (TCPIsPutReady(MySocket) < strlenpgm((const char *) strTitle))
			break;
	...
	case SM_GET_LOGIN:
		...
		if (TCPIsPutReady(MySocket) <
			strlenpgm((const char *) strPassword))
			break;
		...
		w2 = TCPFindROMArray(MySocket, (const unsigned char *) USERNAME,
							 sizeof(USERNAME) - 1, 0, TRUE);
		if ((w2 != 0)
			|| !((sizeof(USERNAME) - 1 == w) || (sizeof(USERNAME) == w))) {
	...
	case SM_GET_PASSWORD_BAD_LOGIN:
		...
		if (TCPIsPutReady(MySocket) <
			strlenpgm((const char *) strAuthenticated))
			break;
		...
		w2 = TCPFindROMArray(MySocket, (const unsigned char *) PASSWORD,
							 sizeof(PASSWORD) - 1, 0, FALSE);
		if ((w2 != 3)
			|| !((sizeof(PASSWORD) - 1 == w - 3)
				 || (sizeof(PASSWORD) == w - 3))
			|| (TelnetState == SM_GET_PASSWORD_BAD_LOGIN)) {
	...
	case SM_AUTHENTICATED:
		if (TCPIsPutReady(MySocket) <
			strlenpgm((const char *) strDisplay) + 4)
			break;
		...
		TCPPutROMString(MySocket, (const unsigned char *) "\x1b[1m");
	...
	case SM_REFRESH_VALUES:
		if (TCPIsPutReady(MySocket) >= 60u) {
			...
			TCPPutROMString(MySocket,
							(const unsigned char *) "\x1b[10;21f");

			// Put analog value with space padding on right side for 4 characters
			TCPPutROMArray(MySocket, (const unsigned char *) "    ",
						   4 - strlen((char *) AN0String));
			TCPPutString(MySocket, AN0String);

			// Put Buttons
			TCPPutROMString(MySocket,
							(const unsigned char *) "\x1b[11;18f");
			...
			// Put LEDs
			TCPPutROMString(MySocket,
							(const unsigned char *) "\x1b[12;10f");
		...
		if (TCPIsGetReady(MySocket)) {
			...
			case 'Q':
				if (TCPIsPutReady(MySocket) >=
					strlenpgm((const char *) strGoodBye))
