...
void TCPTXPerformanceTask(void)
{
	...
	unsigned char vBuffer[10];
	static unsigned char vBytesPerSecond[12];
	WORD w;
	DWORD dw;
	QWORD qw;
	...
	if (MySocket == INVALID_SOCKET) {
		MySocket =
			TCPOpen(0, TCP_OPEN_SERVER, TX_PERFORMANCE_PORT,
	...
	while (w >= 12 + 27 + 5 + 32u) {
		...
		TCPPutROMArray(MySocket, (const unsigned char *) "       ",
					   5 - strlen((char *) vBytesPerSecond));
		TCPPutString(MySocket, vBytesPerSecond);
		TCPPutROMString(MySocket,
						(const unsigned char *)
						"00 bytes/second TX throughput.\r\n");
...
void TCPRXPerformanceTask(void)
{
	...
	if (MySocket == INVALID_SOCKET) {
		MySocket =
			TCPOpen(0, TCP_OPEN_SERVER, RX_PERFORMANCE_PORT,
	...
	if (dw > TICK_SECOND) {
		...
		TCPPutROMString(MySocket,
						(const unsigned char *) " bytes/second\r\n");
