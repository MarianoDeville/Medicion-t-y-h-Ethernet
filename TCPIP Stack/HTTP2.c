 ...
 * String Constants
 ********************************************************************/
static const unsigned char HTTP_CRLF[] = "\r\n";	// New line sequence
...
static const char *httpFileExtensions[HTTP_UNKNOWN + 1] = {
	"txt",						// HTTP_TXT
	"htm",						// HTTP_HTM
	"html",						// HTTP_HTML
	"cgi",						// HTTP_CGI
...
static const char *httpContentTypes[HTTP_UNKNOWN + 1] = {
	"text/plain",				// HTTP_TXT
	"text/html",				// HTTP_HTM
	"text/html",				// HTTP_HTML
	"text/html",				// HTTP_CGI
...
static const char *HTTPResponseHeaders[] = {
	"HTTP/1.1 200 OK\r\nConnection: close\r\n",
	"HTTP/1.1 200 OK\r\nConnection: close\r\n",
...
static const char *HTTPRequestHeaders[HTTP_NUM_HEADERS] = {
	"Cookie:",
	"Authorization:",
	"Content-Length:"
};
 ...
 ********************************************************************/
//#pragma udata HTTP_CONNECTION_STATES// LO SAQUE PORQUE ME DA WARNING, NO SE QUE HACE.////////////////
...
unsigned char curHTTPID;		// Which HTTP stub is active
//#pragma udata	// LO SAQUE PORQUE ME DA WARNING, NO SE QUE HACE////////////////////////////////////
...
static void HTTPHeaderParseLookup(unsigned char i);
...
static void HTTPLoadConn(unsigned char connID);
...
void HTTPInit(void)
{
	...
	for (curHTTPID = 0; curHTTPID < MAX_HTTP_CONNECTIONS; curHTTPID++) {
		...
		oldPtr =
			MACSetWritePtr(BASE_HTTPB_ADDR +
						   curHTTPID * sizeof(HTTP_CONN));
		MACPutArray((unsigned char *) &curHTTP, sizeof(HTTP_CONN));
...
void HTTPServer(void)
{
	...
	for (conn = 0; conn < MAX_HTTP_CONNECTIONS; conn++) {
		...
		if (httpStubs[conn].sm != SM_HTTP_IDLE
			|| TCPIsGetReady(httpStubs[conn].socket)) {
			HTTPLoadConn(conn);
			HTTPProcess();
		}
	}
...
/*********************************************************************
 * Function:        static void HTTPLoadConn(unsigned char connID)
 *
 * PreCondition:    None
 *
...
static void HTTPLoadConn(unsigned char connID)
{
	...
	// Save the old one
	oldPtr =
		MACSetWritePtr(BASE_HTTPB_ADDR + curHTTPID * sizeof(HTTP_CONN));
	MACPutArray((unsigned char *) &curHTTP, sizeof(HTTP_CONN));
	MACSetWritePtr(oldPtr);

	// Load the new one
	oldPtr = MACSetReadPtr(BASE_HTTPB_ADDR + connID * sizeof(HTTP_CONN));
	MACGetArray((unsigned char *) &curHTTP, sizeof(HTTP_CONN));
...
static void HTTPProcess(void)
{
	...
	BOOL isDone;
	unsigned char *ext;
	unsigned char buffer[HTTP_MAX_HEADER_LEN + 1];
	...
	do {
		...
		if (TCPWasReset(sktHTTP)) {
			...
			TCPAdjustFIFOSize(sktHTTP, 1, 0,
							  TCP_ADJUST_GIVE_REST_TO_RX |
							  TCP_ADJUST_PRESERVE_RX);
		}
		...
		case SM_HTTP_IDLE:
			...
			if (lenA) {			// Clear out state info and move to next state
				...
				curHTTP.callbackID =
					TickGet() + HTTP_TIMEOUT * TICK_SECOND;
		...
		case SM_HTTP_PARSE_REQUEST:
			...
			if (TCPFind(sktHTTP, '\n', 0, FALSE) == 0xffff) {	// First line isn't here yet
				if (TCPGetRxFIFOFree(sktHTTP) == 0) {	// If the FIFO is full, we overflowed
					curHTTP.httpStatus = HTTP_OVERFLOW;
					smHTTP = SM_HTTP_SERVE_HEADERS;
					isDone = FALSE;
				}
				if (TickGet() > curHTTP.callbackID) {	// A timeout has occurred
			...
			if (memcmppgm2ram(curHTTP.data, (const void *) "GET", 3) == 0)
				curHTTP.httpStatus = HTTP_GET;
#if defined(HTTP_USE_POST)
			else if (memcmppgm2ram(curHTTP.data, (const void *) "POST", 4)
					 == 0)
...
#if defined(HTTP_MPFS_UPLOAD)
			if (memcmppgm2ram(&curHTTP.data[1], HTTP_MPFS_UPLOAD, strlenpgm(HTTP_MPFS_UPLOAD)) == 0) {	// Read remainder of line, and bypass all file opening, etc.
#if defined(HTTP_USE_AUTHENTICATION)
				curHTTP.isAuthorized =
					HTTPAuthenticate(NULL, NULL, &curHTTP.data[1]);
#endif
			...
			if (curHTTP.file == MPFS_INVALID_HANDLE) {
				...
				if (TCPIsLoopback(sktHTTP)) {
					strcpypgm2ram((void *) &curHTTP.data[lenB],
								  HTTPS_DEFAULT_FILE);
			...
			ext++;
			for (curHTTP.fileType = HTTP_TXT;
				 curHTTP.fileType < HTTP_UNKNOWN; curHTTP.fileType++)
				if (!stricmppgm2ram
					(ext,
					 (const void *) httpFileExtensions[curHTTP.fileType]))
					break;
			...
			if (lenA != 0) {
				...
				curHTTP.ptrData +=
					TCPGetArray(sktHTTP, curHTTP.data, lenA);
				*(curHTTP.ptrData++) = '&';
		...
		case SM_HTTP_PARSE_HEADERS:
			...
			while (1) {
				...
				if (lenA == 0xffff) {	// If not, make sure we can receive more data
					if (TCPGetRxFIFOFree(sktHTTP) == 0) {	// Overflow
						curHTTP.httpStatus = HTTP_OVERFLOW;
						smHTTP = SM_HTTP_SERVE_HEADERS;
						isDone = FALSE;
					}
					if (TickGet() > curHTTP.callbackID) {	// A timeout has occured
						TCPDisconnect(sktHTTP);
						smHTTP = SM_HTTP_DISCONNECT;
						isDone = FALSE;
					}
				...
				if (lenA == 1) {	// Remove the CRLF and move to next state
				...
				for (i = 0; i < HTTP_NUM_HEADERS; i++) {
					if (strcmppgm2ram((char *) buffer, (const char *) HTTPRequestHeaders[i]) == 0) {	// Parse the header and stop the loop
		...
		case SM_HTTP_PROCESS_POST:
			...
			if (TCPIsGetReady(sktHTTP) == curHTTP.callbackPos) {
				if (TickGet() > curHTTP.callbackID) {	// If a timeout has occured, disconnect
					TCPDisconnect(sktHTTP);
					smHTTP = SM_HTTP_DISCONNECT;
					isDone = FALSE;
					break;
			...
			if (curHTTP.httpStatus == HTTP_POST
...
#if defined(HTTP_MPFS_UPLOAD)
				if (curHTTP.httpStatus >= HTTP_MPFS_UP
					&& curHTTP.httpStatus <= HTTP_MPFS_ERROR) {
					c = HTTPMPFSUpload();
					if (c == HTTP_IO_DONE) {
						smHTTP = SM_HTTP_SERVE_HEADERS;
				...
				if (c == HTTP_IO_WAITING) {	// return to main app and make sure we don't get stuck by the watchdog
					curHTTP.callbackPos = TCPIsGetReady(sktHTTP) - 1;
					break;
				} else if (c == HTTP_IO_NEED_DATA) {	// If waiting for more data
					...
					if (curHTTP.byteCount > 0
						&& TCPGetRxFIFOFree(sktHTTP) != 0)
						break;
					else {		// Handle cases where application ran out of data or buffer space
		...
		case SM_HTTP_SERVE_HEADERS:
			...
			TCPPutROMString(sktHTTP,
							(const unsigned char *)
							HTTPResponseHeaders[curHTTP.httpStatus]);
			...
			if (curHTTP.httpStatus == HTTP_POST || curHTTP.nextCallback != 0xffffffff) {	// This is a dynamic page or a POST request, so no cache
				TCPPutROMString(sktHTTP,
								(const unsigned char *) "no-cache");
			} else {			// This is a static page, so save it for the specified amount of time
				TCPPutROMString(sktHTTP,
								(const unsigned char *) "max-age=");
				TCPPutROMString(sktHTTP,
								(const unsigned char *) HTTP_CACHE_LEN);
			}
			TCPPutROMString(sktHTTP, HTTP_CRLF);
		...
		case SM_HTTP_SERVE_COOKIES:
			...
			for (curHTTP.ptrRead = curHTTP.data; curHTTP.hasArgs != 0;
				 curHTTP.hasArgs--) {
				// Write the header
				TCPPutROMString(sktHTTP,
		...
		case SM_HTTP_SEND_FROM_CALLBACK:
			...
			if (curHTTP.callbackPos == 0) {	// Callback finished its output, so move on
				isDone = FALSE;
				smHTTP = SM_HTTP_SERVE_BODY;
...
{
	WORD numBytes, len;
	unsigned char c, data[64];

	// Determine how many bytes we can read right now
	numBytes =
		mMIN(TCPIsPutReady(sktHTTP),
			 curHTTP.nextCallback - curHTTP.byteCount);
...
/*********************************************************************
 * Function:        static void HTTPHeaderParseLookup(unsigned char i)
 *
 * PreCondition:    None
 *
...
static void HTTPHeaderParseCookie(void)
{
	...
	if (lenB >= curHTTP.data + HTTP_MAX_DATA_LEN - curHTTP.ptrData - 2) {	// If not, overflow
		curHTTP.httpStatus = HTTP_OVERFLOW;
		smHTTP = SM_HTTP_SERVE_HEADERS;
		return;
	}
	...
	while (lenB != 0) {
		...
		curHTTP.ptrData +=
			TCPGetArray(sktHTTP, curHTTP.ptrData, mMIN(lenA, lenB));
		...
		*(curHTTP.ptrData++) = '&';
		...
		lenB =
			TCPFindROMArray(sktHTTP, HTTP_CRLF, HTTP_CRLF_LEN, 0, FALSE);
	}
...
/*********************************************************************
 * Function:        unsigned char* HTTPURLDecode(unsigned char *data)
 *
...
/*********************************************************************
 * Function:        unsigned char* HTTPGetArg(unsigned char *data, unsigned char* arg)
 *
 * PreCondition:    None
 *
 ...
 * Side Effects:    None
 *
 * Overview:        Searches a unsigned char array for a named argument and
 *					returns its value.
 *
...
unsigned char *HTTPGetArg(unsigned char *data, unsigned char *arg)
{
	...
	while (*data != '\0') {
		...
		if (!strcmp((char *) arg, (char *) data)) {	// Found it, so return parameter
...
unsigned char *HTTPGetROMArg(unsigned char *data, const unsigned char *arg)
{
	...
	while (*data != '\0') {
		// Look for arg at current position
		if (!memcmppgm2ram(data, (const void *) arg, strlenpgm((const char *) arg) + 1)) {	// Found it, so skip to next string
			return data + strlenpgm((const char *) arg) + 1;
		}
		// Skip past two strings (NUL bytes)
		data += strlen((char *) data) + 1;
		data += strlen((char *) data) + 1;
...
static HTTP_IO_RESULT HTTPMPFSUpload(void)
{
	...
	case HTTP_MPFS_UP:
		...
		if (lenA != 0xffff) {	// Found it, so remove all data up to and including
			...
			curHTTP.byteCount -= lenA;
			if (memcmppgm2ram(c, (const void *) "\r\n\r\nMPFS\x02\x00", 10) == 0) {	// Read as Ver 2.0
	...
	case HTTP_MPFS_ERROR:
		...
		TCPDiscard(sktHTTP);
		if (curHTTP.byteCount < 100 || curHTTP.byteCount > 0x80000000) {	// If almost all data was read, or if we overflowed, then return
			smHTTP = SM_HTTP_SERVE_HEADERS;
			return HTTP_IO_DONE;
		}
		break;
 ...
 *						DWORD callbackPos, const unsigned char* file)
 *
...
void HTTPIncFile(const unsigned char *file)
{
	...
	if (curHTTP.callbackPos == 0x00) {	// On initial call, open the file and save its ID
		fp = MPFSOpenROM(file);
		if (fp == MPFS_INVALID_HANDLE) {	// File not found, so abort
			return;
		}
		((DWORD_VAL *) & curHTTP.callbackPos)->w[0] = MPFSGetID(fp);
	} else {					// The file was already opened, so load up it's ID and seek
		fp = MPFSOpenID(((DWORD_VAL *) & curHTTP.callbackPos)->w[0]);
		if (fp == MPFS_INVALID_HANDLE) {	// File not found, so abort
			curHTTP.callbackPos = 0x00;
			return;
		}
		MPFSSeek(fp, ((DWORD_VAL *) & curHTTP.callbackPos)->w[1],
				 MPFS_SEEK_FORWARD);
	}
	...
	while (count > 0) {
		...
		if (len == 0) {			// If no bytes were read, an EOF was reached
			MPFSClose(fp);
			curHTTP.callbackPos = 0x00;
			return;
