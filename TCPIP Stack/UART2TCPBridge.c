...
static unsigned char vUARTRXFIFO[65];
static unsigned char vUARTTXFIFO[17];
static unsigned char *RXHeadPtr = vUARTRXFIFO, *RXTailPtr = vUARTRXFIFO;
static unsigned char *TXHeadPtr = vUARTTXFIFO, *TXTailPtr = vUARTTXFIFO;
...
#define CLOSEST_SPBRG_VALUE ((INSTR_FREQ+2ul*BAUD_RATE)/BAUD_RATE/4-1)
#define BAUD_ACTUAL (INSTR_FREQ/(CLOSEST_SPBRG_VALUE+1))
...
#define BAUD_ERROR_PRECENT	((BAUD_ERROR*100+BAUD_RATE/2)/BAUD_RATE)
	...
	BAUDCONbits.BRG16 = 1;
	TXSTAbits.BRGH = 1;
	SPBRGH = ((INSTR_FREQ + BAUD_RATE / 2) / BAUD_RATE - 1) >> 8 & 0xFF;
	SPBRG = ((INSTR_FREQ + BAUD_RATE / 2) / BAUD_RATE - 1) & 0xFF;
#else
...
void UART2TCPBridgeTask(void)
{
	...
	unsigned char *RXHeadPtrShadow, *RXTailPtrShadow;
	unsigned char *TXHeadPtrShadow, *TXTailPtrShadow;
	...
	case SM_HOME:
		...
		MySocket =
			TCPOpen((DWORD) USE_REMOTE_TCP_SERVER, TCP_OPEN_ROM_HOST,
					UART2TCPBRIDGE_PORT, TCP_PURPOSE_UART_2_TCP_BRIDGE);
#else
		MySocket =
			TCPOpen(0, TCP_OPEN_SERVER, UART2TCPBRIDGE_PORT,
...
#if defined(__C32__)
void __attribute((interrupt(ipl6), vector(_UART2_VECTOR), nomips16))
U2Interrupt(void)
{
	if (IFS1bits.U2RXIF)
		_U2RXInterrupt();
