...
// Internal functions
static unsigned char *FindEmailAddress(unsigned char *str, WORD * wLen);
static const unsigned char *FindROMEmailAddress(const unsigned char *str,
												WORD * wLen);
...
void SMTPTask(void)
{
	unsigned char i;
	WORD w;
	unsigned char vBase64Buffer[4];
	static TICK Timer;
	static unsigned char RXBuffer[4];
	static const unsigned char *ROMStrPtr, *ROMStrPtr2;
	static unsigned char *RAMStrPtr;
		...
		if (SMTPClient.Server.szRAM || SMTPClient.Server.szROM) {
			...
			if (!(SMTPClient.Server.szRAM || SMTPClient.Server.szROM)) {
				if (SMTPClient.CC.szRAM && !SMTPClient.ROMPointers.CC) {
					SMTPClient.Server.szRAM =
						(unsigned char *) strchr((char *) SMTPClient.CC.
												 szRAM, '@');
					SMTPClient.ROMPointers.Server = 0;
				} else if (SMTPClient.CC.szROM
			...
			if (!(SMTPClient.Server.szRAM || SMTPClient.Server.szROM)) {
				if (SMTPClient.BCC.szRAM && !SMTPClient.ROMPointers.BCC) {
					SMTPClient.Server.szRAM =
						(unsigned char *) strchr((char *) SMTPClient.BCC.
												 szRAM, '@');
					SMTPClient.ROMPointers.Server = 0;
				} else if (SMTPClient.BCC.szROM
						   && SMTPClient.ROMPointers.BCC) {
					SMTPClient.Server.szROM =
						(const unsigned char *) strchrpgm((const char *)
														  SMTPClient.BCC.
														  szROM, '@');
	...
	case TRANSPORT_OBTAIN_SOCKET:
		...
		MySocket =
			TCPOpen(SMTPServer.Val, TCP_OPEN_IP_ADDRESS,
					SMTPClient.ServerPort, TCP_PURPOSE_DEFAULT);

		// Abort operation if no TCP sockets are available
		// If this ever happens, incrementing add some more 
	...
	case TRANSPORT_SOCKET_OBTAINED:
		if (!TCPIsConnected(MySocket)) {
			...
			// to the SMTP server
			if (SMTPFlags.bits.ConnectedOnce
				|| ((LONG) (TickGet() - Timer) >
					(LONG) (SMTP_SERVER_REPLY_TIMEOUT))) {
		...
		case SMTP_HELO:
			TCPPutROMString(MySocket,
							(const unsigned char *) "HELO MCHPBOARD\r\n");
			TCPFlush(MySocket);
			SMTPState++;
			break;
		...
		case SMTP_AUTH_LOGIN:
			...
			TCPPutROMString(MySocket,
							(const unsigned char *) "AUTH LOGIN\r\n");
			TCPFlush(MySocket);
			SMTPState++;
			break;
		...
		case SMTP_AUTH_USERNAME:
			...
			if (SMTPClient.ROMPointers.Username) {
				...
				w = strlenpgm((const char *) ROMStrPtr);
			} else {
			...
			while (w) {
				...
				w -= i;
				Base64Encode(vBase64Buffer, i, vBase64Buffer,
							 sizeof(vBase64Buffer));
				TCPPutArray(MySocket, vBase64Buffer,
							sizeof(vBase64Buffer));
			}
			TCPPutROMString(MySocket, (const unsigned char *) "\r\n");
			TCPFlush(MySocket);
			SMTPState++;
			break;
		...
		case SMTP_MAILFROM:
			...
			// return address.
			TCPPutROMString(MySocket,
							(const unsigned char *) "MAIL FROM:<");
			if (SMTPClient.ROMPointers.From) {
				ROMStrPtr =
					FindROMEmailAddress(SMTPClient.From.szROM,
										&wAddressLength);
				TCPPutROMArray(MySocket, ROMStrPtr, wAddressLength);
			} else {
				RAMStrPtr =
					FindEmailAddress(SMTPClient.From.szRAM,
									 &wAddressLength);
		...
		case SMTP_RCPTTO_INIT:
			...
			if (SMTPClient.To.szRAM && !SMTPClient.ROMPointers.To) {
				RAMStrPtr =
					FindEmailAddress(SMTPClient.To.szRAM, &wAddressLength);
				if (wAddressLength) {
			...
			if (SMTPClient.To.szROM && SMTPClient.ROMPointers.To) {
				ROMStrPtr =
					FindROMEmailAddress(SMTPClient.To.szROM,
										&wAddressLength);
				if (wAddressLength) {
					SMTPState = SMTP_RCPTTO;
					break;
		...
		case SMTP_RCPTTOCC:
		case SMTP_RCPTTOBCC:
			TCPPutROMString(MySocket, (const unsigned char *) "RCPT TO:<");
		...
		case SMTP_RCPTTO_ISDONE:
			...
			if (SMTPClient.ROMPointers.To)
				ROMStrPtr =
					FindROMEmailAddress(ROMStrPtr + wAddressLength,
										&wAddressLength);
			else
				RAMStrPtr =
					FindEmailAddress(RAMStrPtr + wAddressLength,
									 &wAddressLength);
		...
		case SMTP_RCPTTOCC_INIT:
			...
			if (SMTPClient.CC.szRAM && !SMTPClient.ROMPointers.CC) {
				RAMStrPtr =
					FindEmailAddress(SMTPClient.CC.szRAM, &wAddressLength);
				if (wAddressLength) {
					SMTPState = SMTP_RCPTTOCC;
					break;
				}
			...
			if (SMTPClient.CC.szROM && SMTPClient.ROMPointers.CC) {
				ROMStrPtr =
					FindROMEmailAddress(SMTPClient.CC.szROM,
										&wAddressLength);
				if (wAddressLength) {
					SMTPState = SMTP_RCPTTOCC;
					break;
		...
		case SMTP_RCPTTOBCC_INIT:
			...
			if (SMTPClient.BCC.szRAM && !SMTPClient.ROMPointers.BCC) {
				RAMStrPtr =
					FindEmailAddress(SMTPClient.BCC.szRAM,
									 &wAddressLength);
				if (wAddressLength) {
					SMTPState = SMTP_RCPTTOBCC;
					break;
			...
			if (SMTPClient.BCC.szROM && SMTPClient.ROMPointers.BCC) {
				ROMStrPtr =
					FindROMEmailAddress(SMTPClient.BCC.szROM,
										&wAddressLength);
				if (wAddressLength) {
					SMTPState = SMTP_RCPTTOBCC;
					break;
		...
		case SMTP_DATA:
			TCPPutROMString(MySocket, (const unsigned char *) "DATA\r\n");
			SMTPState++;
		...
		case SMTP_DATA_HEADER:
			while ((PutHeadersState != PUTHEADERS_DONE)
				   && (TCPIsPutReady(MySocket) > 64u)) {
				...
				case PUTHEADERS_TO:
					if (SMTPClient.ROMPointers.To) {
						...
						SMTPClient.To.szRAM =
							TCPPutString(MySocket, SMTPClient.To.szRAM);
				...
				case PUTHEADERS_CC:
					if (SMTPClient.ROMPointers.CC) {
						...
						SMTPClient.CC.szRAM =
							TCPPutString(MySocket, SMTPClient.CC.szRAM);
		...
		case SMTP_DATA_BODY_INIT:
			...
			ROMStrPtr2 = (const unsigned char *) "\r\n.\r\n";
			CRPeriod.Pos = NULL;
			if (RAMStrPtr)
				CRPeriod.Pos =
					(unsigned char *) strstrrampgm((char *) RAMStrPtr,
												   (const char *) "\r\n.");
			// No break here
			...
			if (SMTPClient.Body.szRAM || SMTPClient.Body.szROM) {
				if (*ROMStrPtr2) {
					...
					while (CRPeriod.Pos) {
						CRPeriod.Pos += 3;
						RAMStrPtr +=
							TCPPutArray(MySocket, RAMStrPtr,
						...
						CRPeriod.Pos =
							(unsigned char *) strstrrampgm((char *)
														   RAMStrPtr,
														   (const char *)
														   "\r\n.");
					}
		...
		case SMTP_QUIT_INIT:
			SMTPState++;
			ROMStrPtr = (const unsigned char *) "QUIT\r\n";
			// No break here
...
static unsigned char *FindEmailAddress(unsigned char *str, WORD * wLen)
{
	unsigned char *lpStart;
	unsigned char c;
	union {
		unsigned char Val;
		struct {
			unsigned char FoundOpenBracket:1;
			unsigned char FoundAt:1;
		} bits;
	} ParseStates;
