...
typedef struct _MPFS_ENTRY {
	unsigned char Flag __attribute__ ((__packed__));
	MPFS Address __attribute__ ((__packed__));
	unsigned char Name[MAX_FILE_NAME_LEN] __attribute__ ((__packed__));
} MPFS_ENTRY;
#else							//Use program memory
typedef struct _MPFS_ENTRY {
	unsigned char Flag;
	MPFS Address;
	unsigned char Name[MAX_FILE_NAME_LEN];
} MPFS_ENTRY;
#if defined(__C30__)
...
unsigned char mpfsOpenCount;

#if !defined(MPFS_USE_EEPROM)
	// An address where MPFS data starts in program memory.
extern const MPFS_ENTRY MPFS_Start[];
#else
...
/*********************************************************************
 * Function:        MPFS MPFSOpen(unsigned char* file)
 *
 * PreCondition:    None
 *
 ...
 * Note:            None
 ********************************************************************/
MPFS MPFSOpen(unsigned char *file)
{
	MPFS_ENTRY entry;
	MPFS FAT;
	...
	file = (unsigned char *) strupr((char *) file);
	...
	while (1) {
...
#else
#if defined(__C30__)
		memcpypgm2ram(&entry, (const void *) (WORD) FAT, sizeof(entry));
#else
		...
		if (entry.Flag == MPFS_DATA) {
			...
			if (memcmp((void *) file, (void *) entry.Name, fileNameLen) ==
				0) {
...
MPFS MPFSOpenROM(const unsigned char *file)
{
	unsigned char nameRAM[MAX_FILE_NAME_LEN + 1];

	memcpypgm2ram(nameRAM, (const void *) file,
				  strlenpgm((const char *) file));
	nameRAM[strlenpgm((const char *) file)] = '\0';
...
/*********************************************************************
 * Function:        unsigned char MPFSGet(void)
 *
 * PreCondition:    MPFSOpen() != MPFS_INVALID &&
 *                  MPFSGetBegin() == TRUE
 *
...
#else
	t = (unsigned char) *_currentHandle;
	_currentHandle++;
#endif
#endif
...
BOOL MPFSPutBegin(MPFS handle)
{
	...
	_currentCount = (unsigned char) handle;
...
/*********************************************************************
 * Function:        BOOL MPFSPut(unsigned char b)
 *
...
{
	unsigned char *b = (unsigned char *) ul;
	*(b) = MPFSGet();
	if (MPFSIsEOF())
		return FALSE;
	*(b + 1) = MPFSGet();
