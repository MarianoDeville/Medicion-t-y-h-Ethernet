...
/*********************************************************************
 * Function:        void HashAddData(HASH_SUM *theSum, unsigned char *data, WORD len)
 *
...
#if defined(__18CXX)
void HashAddROMData(HASH_SUM * theSum, const unsigned char *data, WORD len)
{
#if defined(STACK_USE_MD5)
	if (theSum->hashType == HASH_MD5)
...
static const unsigned char _MD5_r[64] =
...
static void MD5HashBlock(unsigned char *data, DWORD * h0, DWORD * h1,
						 DWORD * h2, DWORD * h3);
...
/*********************************************************************
 * Function:        void MD5AddData(HASH_SUM *theSum, unsigned char *data, WORD len)
 *
	...
	while (len != 0) {
		*blockPtr++ = *data++;
		...
		if (blockPtr == theSum->partialBlock + 64) {
			MD5HashBlock(theSum->partialBlock, &theSum->h0, &theSum->h1,
						 &theSum->h2, &theSum->h3);
...
/*********************************************************************
 * Function:        static void MD5HashBlock(unsigned char *data, DWORD *h0, 
						 ...
						 DWORD * h2, DWORD * h3)
{
	...
	for (i = 0; i < 64; i++) {
		...
		c = b;
		j *= 4;
		b = leftRotateDWORD(a + f + _MD5_k[i] + (*(DWORD *) & data[j]),
							_MD5_r[i]) + b;
		a = temp;
	}
...
/*********************************************************************
 * Function:        void MD5Calculate(HASH_SUM *theSum, unsigned char *result)
 *
 * PreCondition:    theSum has been initialized with MD5Initialize() 
 *
 * Input:           *theSum: A pointer to an HASH_SUM struct to use
...
{
	DWORD h0, h1, h2, h3;
	unsigned char i, *partPtr, *endPtr;
	...
	if (i > 56) {				// If there's not enough space, then zero fill this and add a new block
...
static void SHA1HashBlock(unsigned char *data, DWORD * h0, DWORD * h1,
...
/*********************************************************************
 * Function:        void SHA1AddData(HASH_SUM *theSum, unsigned char *data, WORD len)
 *
...
#if defined(__18CXX)
void SHA1AddROMData(HASH_SUM * theSum, const unsigned char *data, WORD len)
{
	unsigned char *blockPtr;
...
/*********************************************************************
 * Function:        static void SHA1HashBlock(unsigned char *data, DWORD *h0, 
						  ...
						  DWORD * h2, DWORD * h3, DWORD * h4)
{
	...
	unsigned char i, back3, back8, back14;
...
#else
			w[i & 0x0f].Val =
				leftRotateDWORD((w[back3].Val ^ w[back8].Val ^ w[back14].
								 Val ^ w[i & 0x0f].Val), 1);
#endif
			back3 += 1;
			back8 += 1;
