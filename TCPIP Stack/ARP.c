...
/*********************************************************************
 * Function:        static BOOL ARPPut(ARP_PACKET *packet)
 *
 * PreCondition:    None
 *
 * Input:           packet: ARP_PACKET structure with correct 
 *							operation and target preconfigured
 *
...
static BOOL ARPPut(ARP_PACKET * packet)
{
	...
	MACPutArray((unsigned char *) packet, sizeof(*packet));
	MACFlush();
	return TRUE;
}
...
BOOL ARPProcess(void)
{
	...
	{
		case SM_ARP_IDLE:
			MACGetArray((unsigned char *) &packet, sizeof(packet));			// Obtain the incoming ARP packet
			MACDiscardRx();
			SwapARPPacket(&packet);
		...
		case SM_ARP_REPLY:
			...
			if (!ARPPut(&packet))			// Send an ARP response to a previously received request
				return FALSE;
			smARP = SM_ARP_IDLE;			// Begin listening for ARP requests again
			break;
	}
	return TRUE;
}
...
void ARPResolve(IP_ADDR * IPAddr)
{
	...
	packet.TargetIPAddr =
		((AppConfig.MyIPAddr.Val ^ IPAddr->Val) & AppConfig.MyMask.
		 Val) ? AppConfig.MyGateway : *IPAddr;
	ARPPut(&packet);
}
#endif
...
BOOL ARPIsResolved(IP_ADDR * IPAddr, MAC_ADDR * MACAddr)
{
	if ((Cache.IPAddr.Val==IPAddr->Val)||((Cache.IPAddr.Val==AppConfig.MyGateway.Val)
		 &&((AppConfig.MyIPAddr.Val^IPAddr->Val)&AppConfig.MyMask.Val)))
	{
