...
typedef struct _SNMP_NOTIFY_INFO {
	...
	unsigned char communityLen;
	SNMP_ID agentIDVar;
	unsigned char notificationCode;
	UDP_SOCKET socket;
	DWORD_VAL timestamp;
} SNMP_NOTIFY_INFO;
...
static SNMP_ACTION ProcessHeader(char *community, unsigned char *len);
static BOOL ProcessGetSetHeader(DWORD * requestID);
static BOOL ProcessVariables(char *community,
							 unsigned char len,
							 DWORD_VAL * request, unsigned char pduType);

static BOOL OIDLookup(unsigned char *oid, unsigned char oidLen,
					  OID_INFO * rec);
static BOOL IsValidCommunity(char *community, unsigned char *len);
static BOOL IsValidInt(DWORD * val);
static BOOL IsValidPDU(SNMP_ACTION * pdu);
static unsigned char IsValidLength(WORD * len);
static BOOL IsASNNull(void);
static BOOL IsValidOID(unsigned char *oid, unsigned char *len);
static unsigned char IsValidStructure(WORD * dataLen);
static void _SNMPDuplexInit(UDP_SOCKET socket);
static void _SNMPPut(unsigned char v);
static unsigned char _SNMPGet(void);
...
static unsigned char ProcessGetVar(OID_INFO * rec, BOOL bAsOID);
static unsigned char ProcessGetNextVar(OID_INFO * rec);
static BOOL GetOIDStringByAddr(OID_INFO * rec, unsigned char *oidString,
							   unsigned char *len);
static unsigned char ProcessSetVar(OID_INFO * rec,
								   SNMP_ERR_STATUS * errorStatus);
static void SetErrorStatus(WORD errorStatusOffset, WORD errorIndexOffset,
						   SNMP_ERR_STATUS errorStatus,
						   unsigned char errorIndex);

// This function is used only when TRAP is enabled.
#if !defined(SNMP_TRAP_DISABLED)
static BOOL GetOIDStringByID(SNMP_ID id, OID_INFO * info,
							 unsigned char *oidString, unsigned char *len);
#endif
...
BOOL SNMPTask(void)
{
	...
	hMIBFile = MPFSOpen((unsigned char *) snmpBIBFile);
...
void SNMPNotifyPrepare(IP_ADDR * remoteHost,
					   ...
					   unsigned char communityLen,
					   SNMP_ID agentIDVar,
					   unsigned char notificationCode, DWORD timestamp)
{
...
BOOL SNMPIsNotifyReady(IP_ADDR * remoteHost)
{
	...
	if (ARPIsResolved(remoteHost, &remoteNode.MACAddr)) {
		...
		SNMPNotifyInfo.socket =
			UDPOpen(AGENT_NOTIFY_PORT, &remoteNode, SNMP_NMS_PORT);
 ...
 *                            unsigned char, WORD or DWORD can be sent.
...
{
	unsigned char len;
	unsigned char OIDValue[OID_MAX_LEN];
	unsigned char OIDLen;
	unsigned char agentIDLen;
	unsigned char *pOIDValue;
	OID_INFO rec;
	...
	_SNMPPut(ASN_OID);
	_SNMPPut((unsigned char) (OIDLen + 1));
	len = OIDLen;
	pOIDValue = OIDValue;
	while (len--)
		_SNMPPut(*pOIDValue++);
	...
	len = len + 2				// Var bind struct header
...
static BOOL ProcessVariables(char *community, unsigned char len,
							 DWORD_VAL * request, unsigned char pduType)
{
	unsigned char temp;
	...
	while (varBindingLen.Val) {
		...
		if (pduType != (unsigned char) SET_REQUEST) {
			if (!IsASNNull())
				return FALSE;
		}
		...
		if (!OIDLookup(OIDValue, OIDLen, &OIDInfo)) {
			...
			SetErrorStatus(errorStatusOffset,
						   errorIndexOffset, SNMP_NO_SUCH_NAME, varIndex);
...
static unsigned char ProcessGetNextVar(OID_INFO * rec)
{
	...
	unsigned char ref;
	static SNMP_VAL v;
	static unsigned char varDataType;
	static unsigned char indexBytes;
	...
	if (rec->nodeInfo.Flags.bIsParent ||
		(!rec->nodeInfo.Flags.bIsParent
		 && !rec->nodeInfo.Flags.bIsSequence)) {
	  _GetNextLeaf:
		lbNextLeaf = TRUE;
		if (!GetNextLeaf(rec))
			return 0;
								   ...
								   SNMP_ERR_STATUS * errorStatus)
{
	...
	if (actualDataTypeInfo.asnLen != 0xff) {
		...
		if (errorCode == SNMP_NO_ERR)
			SNMPSetVar(rec->id, rec->index, (WORD) SNMP_END_OF_VAR,
					   dataValue);
	}
						   ...
						   unsigned char errorIndex)
{
	...
	_SNMPPut((unsigned char) errorStatus);
...
BOOL SNMPTask(void)
{
	...
	// Open MIB file.
	SNMPStatus.Flags.bIsFileOpen = FALSE;
	hMPFS = MPFSOpenROM((const unsigned char *) SNMP_BIB_FILE_NAME);
...
static unsigned char ProcessGetNextVar(OID_INFO * rec)
{
	...
	// Fetch index data type.
	MPFSGet(hMPFS, (unsigned char *) &indexRec.dataType);
	...
	while (1) {
		...
		if (tempOID != *oid) {
			...
			if (!rec->nodeInfo.Flags.bIsParent) {
				...
				MPFSGet(hMPFS, (unsigned char *) &rec->dataType);
...
static BOOL GetNextLeaf(OID_INFO * n)
{
	...
	while (1) {
		...
		// Fetch data type.
		MPFSGet(hMPFS, (unsigned char *) &n->dataType);
