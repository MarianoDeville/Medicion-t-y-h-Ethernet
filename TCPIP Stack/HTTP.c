...
typedef struct _FILE_TYPES {
	unsigned char fileExt[FILE_EXT_LEN + 1];
} FILE_TYPES;
...
static const FILE_TYPES httpFiles[] = {
	{"TXT"},					// HTTP_TXT
	{"HTM"},					// HTTP_HTML
	{"CGI"},					// HTTP_CGI
	{"XML"},					// HTTP_XML
...
typedef struct _HTTP_CONTENT {
	const unsigned char typeString[20];
} HTTP_CONTENT;

// Content entry order must match with those "HTTP File Types" define's.
static const HTTP_CONTENT httpContents[] = {
	{"text/plain"},				// HTTP_TXT
	{"text/html"},				// HTTP_HTML
	{"text/html"},				// HTTP_CGI
	{"text/xml"},				// HTTP_XML
...
typedef struct _HTTP_INFO {
	...
	MPFS file;
	SM_HTTP smHTTP;
	unsigned char smHTTPGet;
	WORD VarRef;
	unsigned char bProcess;
	unsigned char Variable;
	unsigned char fileType;
} HTTP_INFO;
typedef unsigned char HTTP_HANDLE;
...
static const unsigned char *HTTPMessages[] = {
	(const unsigned char *) "HTTP/1.1 404 Not found\r\n\r\nNot found.\r\n",
	(const unsigned char *) "HTTP/1.1 503 \r\n\r\nService Unavailable\r\n"
};

// Standard HTTP messages.
static const unsigned char HTTP_OK_STRING[] =
	"HTTP/1.1 200 OK\r\nContent-type: ";
static const unsigned char HTTP_OK_NO_CACHE_STRING[] =
...
static const unsigned char HTTP_HEADER_END_STRING[] = "\r\n\r\n";
#define HTTP_HEADER_END_STRING_LEN (sizeof(HTTP_HEADER_END_STRING)-1)

// HTTP Command Strings
static const unsigned char HTTP_GET_STRING[] = "GET";
#define HTTP_GET_STRING_LEN (sizeof(HTTP_GET_STRING)-1)

// Default HTML file.
static const unsigned char HTTP_DEFAULT_FILE_STRING[] = "INDEX.HTM";
...
static HTTP_COMMAND HTTPParse(unsigned char *string,
							  unsigned char **arg,
							  unsigned char *argc, unsigned char *type);
...
static void HTTPProcess(HTTP_HANDLE h)
{
	unsigned char httpData[MAX_HTML_CMD_LEN + 1];
	HTTP_COMMAND httpCommand;
	BOOL lbContinue;
	unsigned char *arg[MAX_HTTP_ARGS];
	unsigned char argc;
	unsigned char i;
	HTTP_INFO *ph;
	WORD w;
	...
	do {
		...
		case SM_HTTP_IDLE:
			...
			w = TCPFindROMArray(ph->socket, (const unsigned char *) "\r\n",
								2, 0, FALSE);
			if (w == 0xFFFFu) {
		...
		case SM_HTTP_NOT_FOUND:
			if (TCPIsPutReady(ph->socket) >=
				strlenpgm((const char *) HTTPMessages[ph->Variable])) {
		...
		case SM_HTTP_HEADER:
			if (TCPIsPutReady(ph->socket)) {
				...
				if (ph->fileType == HTTP_DYNAMIC_FILE_TYPE) {
					ph->bProcess = TRUE;
					TCPPutROMArray(ph->socket,
								   (const unsigned char *)
								   HTTP_OK_NO_CACHE_STRING,
								   HTTP_OK_NO_CACHE_STRING_LEN);
				} else {
					ph->bProcess = FALSE;
					TCPPutROMArray(ph->socket,
								   (const unsigned char *) HTTP_OK_STRING,
								   HTTP_OK_STRING_LEN);
				}

				TCPPutROMString(ph->socket,
								httpContents[ph->fileType].typeString);
				TCPPutROMArray(ph->socket,
							   (const unsigned char *)
							   HTTP_HEADER_END_STRING,
							   HTTP_HEADER_END_STRING_LEN);
...
/*********************************************************************
 * Function:        static HTTP_COMMAND HTTPParse(unsigned char *string,
 *                                              unsigned char** arg,
 *                                              unsigned char* argc,
 *                                              unsigned char* type)
 *
							  ...
							  unsigned char *argc, unsigned char *type)
{
	...
	if (!memcmppgm2ram
		(string, (const void *) HTTP_GET_STRING, HTTP_GET_STRING_LEN)) {
	...
	*type = HTTP_UNKNOWN;
	if (ext != NULL) {
		ext = (unsigned char *) strupr((char *) ext);

		fileType = httpFiles[0].fileExt;
		for (c = 0; c < TOTAL_FILE_TYPES; c++) {
			if (!memcmppgm2ram
				((void *) ext, (const void *) fileType, FILE_EXT_LEN)) {
				*type = c;
				break;
			}
	...
	if (i == 0u) {
		memcpypgm2ram(arg[0], (const void *) HTTP_DEFAULT_FILE_STRING,
