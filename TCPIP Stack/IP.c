...
static unsigned char IPHeaderLen;
...
BOOL IPGetHeader(IP_ADDR * localIP,
				 NODE_INFO * remote, unsigned char *protocol, WORD * len)
{
	...
	WORD checksums[2];
	unsigned char optionsLen;
#define MAX_OPTIONS_LEN     (40)	// As per RFC 791.
	unsigned char options[MAX_OPTIONS_LEN];
#endif
	...
	checksums[0] =
		~CalcIPChecksum((unsigned char *) &header, sizeof(header));

	// Calculate Options checksum too, if they are present.
	if (optionsLen > 0)
		checksums[1] =
			~CalcIPChecksum((unsigned char *) options, optionsLen);
	else
		checksums[1] = 0;

	CalcChecksum.Val = CalcIPChecksum((unsigned char *) checksums,
									  2 * sizeof(WORD));
 ...
 *                  time.
 ********************************************************************/
WORD IPPutHeader(NODE_INFO * remote, unsigned char protocol, WORD len)
{
	IP_HEADER header;
